...
const estadoColors = {
  ...
  RESERVADA: 'bg-amber-50 text-amber-700 ring-1 ring-amber-600/10',
  CON_PAGO: 'bg-sky-50 text-sky-700 ring-1 ring-sky-600/10',
  TRANSFERIDA: 'bg-violet-50 text-violet-700 ring-1 ring-violet-600/10',
  ANULADA: 'bg-red-50 text-red-700 ring-1 ring-red-600/10'
}
...
export default function BoletaList({ boletas, loading }: BoletaListProps) {
  ...
  const filteredBoletas = useMemo(() => {
    ...
    return boletas.filter(boleta => 
      boleta.numero.toString().padStart(4, '0').includes(searchLower) ||
      (boleta.cliente_info?.nombre.toLowerCase().includes(searchLower) || false) ||
      (boleta.cliente_info?.identificacion.includes(searchTerm) || false)
    )
  }, [boletas, searchTerm])
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1">
            <input
              ...
              className="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none text-slate-900 placeholder-slate-400 bg-white transition-all"
            />
          </div>
          
          <div className="flex items-center gap-2">
            <label htmlFor="itemsPerPage" className="text-sm text-slate-500 font-medium">
              Mostrar:
            </label>
            <select
              id="itemsPerPage"
              ...
              className="px-3 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none text-slate-900 bg-white transition-all"
            >
              ...
              {paginatedBoletas.length === 0 ? (
                ...
                paginatedBoletas.map((boleta) => (
                  <tr key={boleta.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${estadoColors[boleta.tipo_estado]}`}>
                        {estadoLabels[boleta.tipo_estado]}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-900">
                        {boleta.cliente_info ? boleta.cliente_info.nombre : '-'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-900">
                        {boleta.cliente_info ? boleta.cliente_info.telefono : '-'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-900">
                        {boleta.cliente_info ? boleta.cliente_info.identificacion : '-'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-900">
                        {boleta.vendedor_info ? boleta.vendedor_info.nombre : '-'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {boleta.qr_url ? (
                        <button
                          onClick={() => window.open(boleta.qr_url || '', '_blank')}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex space-x-2">
                        <button
                          onClick={() => router.push(`/boletas/${boleta.id}`)}
                          className="text-indigo-600 hover:text-indigo-800 text-xs font-semibold flex items-center gap-1 transition-colors"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button
                          onClick={() => router.push(`/boletas/${boleta.id}/print`)}
                          className="text-emerald-600 hover:text-emerald-800 text-xs font-semibold flex items-center gap-1 transition-colors"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="px-3 py-1 text-sm border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed text-slate-700"
              >
                Anterior
              </button>
