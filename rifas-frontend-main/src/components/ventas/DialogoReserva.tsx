      ...
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
            <h3 className="font-medium text-indigo-900 mb-3">Boletas a Reservar</h3>
            <div className="space-y-2">
              {boletas.slice(0, 3).map((boleta) => (
                <div key={boleta.id} className="flex justify-between text-sm">
                  <span className="text-indigo-800">#{boleta.numero.toString().padStart(4, '0')}</span>
                  <span className="text-indigo-600 font-medium">${precioBoleta.toLocaleString()}</span>
                </div>
              ))}
              {boletas.length > 3 && (
            ...
            <div className="border-t border-indigo-200 mt-3 pt-3">
              <div className="flex justify-between font-medium">
                <span className="text-indigo-900">Total ({boletas.length} boletas)</span>
                <span className="text-indigo-900">${subtotal.toLocaleString()}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="mt-2 text-center">
                  <span className="text-2xl font-bold text-indigo-600">{diasBloqueo}</span>
        ...
        <div className="sticky bottom-0 bg-white border-t border-slate-200 p-6 flex gap-3">
          ...
          <button
            ...
            className="flex-1 px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl hover:from-indigo-700 hover:to-indigo-800 font-semibold shadow-md shadow-indigo-500/20 transition-all disabled:opacity-50 font-medium transition-colors"
          >
