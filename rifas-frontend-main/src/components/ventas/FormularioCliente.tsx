...
export default function FormularioCliente({ 
  ...
  const buscarCliente = async (query: string) => {
    ...
    setBuscando(true)
    try {
      const response = await ventasApi.buscarCliente(query)
      setResultadosBusqueda(response.data)
      setMostrarResultados(response.data.length > 0)
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-slate-900 focus:border-transparent outline-none ${
