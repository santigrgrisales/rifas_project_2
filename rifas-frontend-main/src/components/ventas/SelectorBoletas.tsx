  ...
  const handleBeforeUnload = () => {
    if (boletasSeleccionadas.length > 0) {
      navigator.sendBeacon(
        `${API_BASE_URL}/api/boletas/unblock-multiple`,
        JSON.stringify({
        ...
        <div className="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {boletasSeleccionadas.map((boleta) => (
              <div
                key={boleta.id}
                className="bg-white border border-indigo-300 rounded-lg p-3 flex items-center justify-between"
              >
                <div>
                  <div className="font-medium text-indigo-900">#{boleta.numero}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {boletasPagina.map((boleta) => (
            <button
              ...
              className="bg-white border-2 border-slate-200 rounded-lg p-4 hover:border-indigo-500 hover:bg-indigo-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
