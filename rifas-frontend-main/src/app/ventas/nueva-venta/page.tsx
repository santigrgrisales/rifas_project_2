...
function SelectorRifa({ rifas, rifaSeleccionada, onRifaSeleccionada }: { 
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6">
      <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Seleccionar Rifa</h2>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-8 h-8 bg-indigo-100 rounded-full mb-4">
        ...
        {error ? (
          ...
          <div className="space-y-8">
            ...
            <div className="flex items-center justify-center space-x-2 mb-8">
              <div className={`flex items-center space-x-2 px-3 py-2 rounded-full text-sm font-semibold transition-colors ${
                pasoActual === 'seleccionar-rifa' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/20' : 'bg-slate-100 text-slate-500'
              }`}>
                <span>1</span>
              ...
              <div className={`flex items-center space-x-2 px-3 py-2 rounded-full text-sm font-semibold transition-colors ${
                pasoActual === 'seleccionar-boletas' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/20' : 'bg-slate-100 text-slate-500'
              }`}>
                <span>2</span>
              ...
              <div className={`w-8 h-px bg-slate-300`}></div>
              <div className={`flex items-center space-x-2 px-3 py-2 rounded-full text-sm font-semibold transition-colors ${
                pasoActual === 'datos-cliente' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/20' : 'bg-slate-100 text-slate-500'
              }`}>
                <span>3</span>
              ...
              <div className={`w-8 h-px bg-slate-300`}></div>
              <div className={`flex items-center space-x-2 px-3 py-2 rounded-full text-sm font-semibold transition-colors ${
                pasoActual === 'resumen' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/20' : 'bg-slate-100 text-slate-500'
              }`}>
                <span>4</span>
                <span>Resumen</span>
              </div>
                ...
                <div className="flex justify-between">
                  ...
                  {boletasSeleccionadas.length > 0 && (
                    <button
                      ...
                      className="px-6 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl hover:from-indigo-700 hover:to-indigo-800 flex items-center space-x-2 font-semibold shadow-md shadow-indigo-500/20 transition-all"
                    >
            ...
            {pasoActual === 'resumen' && (!cliente || cliente.nombre.trim() === '') && (
              <div className="text-center py-12">
                ...
                <button
                  onClick={() => setPasoActual('datos-cliente')}
                  className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl hover:from-indigo-700 hover:to-indigo-800 font-semibold shadow-md shadow-indigo-500/20 transition-all"
                >
                  Seleccionar Cliente
                </button>
              </div>
